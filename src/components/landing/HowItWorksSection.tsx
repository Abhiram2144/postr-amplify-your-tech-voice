      ...
      case "analysis":
        return <motion.div initial={{
          ...
          ease: "easeOut"
            ...
            {step.preview.metrics?.map((metric, i) => <motion.div key={metric} initial={{
            opacity: 0,
            x: -10
          }} animate={{
            opacity: 1,
      ...
      case "output":
        return <motion.div initial={{
          ...
          ease: "easeOut"
            ...
            {step.preview.platforms?.map((platform, i) => <motion.div key={platform} initial={{
            opacity: 0,
            scale: 0.9
          }} animate={{
            opacity: 1,
        ...
        <div className="container relative mx-auto flex h-full items-center px-4">
          <div className="w-full py-24 sm:py-32">
            ...
            <div className="relative flex justify-between">
              ...
              <motion.div className="absolute left-[28px] right-[28px] top-[28px] h-1 -translate-y-1/2 rounded-full bg-gradient-to-r from-primary to-accent" initial={{
              scaleX: 0
            }} animate={{
              ...
              return <motion.button key={index} onClick={() => handleStepClick(index)} className="group relative z-10 flex flex-col items-center" animate={{
                ...
                ease: "easeOut"
                    ...
                    {/* Step circle */}
                    <div className={`flex h-14 w-14 items-center justify-center rounded-full border-2 transition-all duration-300 ${isActive ? "border-primary bg-primary text-primary-foreground shadow-lg" : isCompleted ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/50"}`} style={isActive ? {
                  boxShadow: "0 0 20px hsl(264, 100%, 65%, 0.4)"
                    ...
                    {/* Step title */}
                    <span className={`mt-3 hidden text-sm font-medium transition-colors sm:block ${isActive ? "text-primary" : isUpcoming ? "text-muted-foreground/50" : "text-muted-foreground"}`}>
                      {step.title}
                    </span>
                  </motion.button>;
            })}
